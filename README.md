...
![A shot generated by reelforge: a black cat walking toward the camera down a neon-lit alley](docs/sample-shot.gif)

*Real output, not a mockup. MiniMax H3 (Hailuo) Max Turbo, text-to-video, 480P,
5 seconds, $0.031 charged — trimmed to 3s and converted to a GIF for this page.*

![The reelforge dashboard, showing a storyboard awaiting per-shot approval](docs/storyboard-board.png)

*A job stopped at its approval gate: the agent planned four beat-aligned shots
and drafted keyframe candidates for each; shot 1 is approved, the rest wait for
a pick. Each card shows the exact prompts that will be used and that shot's
estimated cost, the header tracks spend against the job budget and the
machine-wide cap, and no video is paid for until every shot here is approved.
(From a `--dry-run`, so the candidates are placeholders; the UI is Chinese.)*
